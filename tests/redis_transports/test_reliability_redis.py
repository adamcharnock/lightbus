...
async def test_random_failures(bus: lightbus.BusNode, caplog):
    pass
    # if random() < 0.3 or (call_id == 0 and call_id not in event_mayhem_ids):
...
async def test_first_event_fails(bus: lightbus.BusNode, caplog, fire_dummy_events, listen_for_events):
    ...
    async def listener(**kwargs):
        ...
        if call_id == 0 and call_id not in event_mayhem_ids:
            # Cause some mayhem
    ...
    assert duplicate_calls > 15
    assert len(event_mayhem_ids) > 0


@pytest.mark.run_loop  # TODO: Have test repeat a few times
async def test_last_event_fails(bus: lightbus.BusNode, caplog):
    pass
